...
<div class="space-y-3">
	...
	<!-- Editor (prominent) -->
	{#if task.editor}
		<div class="flex items-center gap-3 p-2 bg-purple-500/10 border border-purple-500/30 rounded-lg">
			{#if task.editor.avatar_url}
				<img src={task.editor.avatar_url} alt="" class="w-10 h-10 rounded-full" />
			{:else}
				<div class="w-10 h-10 rounded-full bg-purple-500 flex items-center justify-center font-medium">
					{task.editor.first_name[0]}{task.editor.last_name[0]}
				</div>
			{/if}
			<div class="flex-1">
				<div class="font-medium">{task.editor.first_name} {task.editor.last_name}</div>
				<div class="text-xs text-purple-400">Редактор</div>
			</div>
			{#if task.editor.telegram_username}
				<a 
					href="https://t.me/{task.editor.telegram_username}" 
					target="_blank"
					class="p-2 hover:bg-purple-500/20 rounded-full transition-colors"
